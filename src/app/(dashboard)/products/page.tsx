...
const CATS = [{ v: '', l: 'Tanpa kategori' }, { v: 'supplement', l: 'Suplemen' }, { v: 'apparel', l: 'Apparel' }, { v: 'accessories', l: 'Aksesoris' }]
...
export default function ProductsPage() {
  ...
  const addProduct = async (e: React.FormEvent) => {
    ...
    const r = await fetch('/api/products', {
      ...
      body: JSON.stringify({ name: form.name, price: Number(form.price), stock: Number(form.stock), category: form.category }),
    })
    setSaving(false)
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={addProduct} className="bg-white rounded-xl p-6 shadow-sm border grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
        ...
        <div>
          ...
          <input type="number" min={0} value={form.price} onChange={(e) => setForm({ ...form, price: Number(e.target.value) })} className={formStyle} required />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Stok</label>
          <input type="number" min={0} value={form.stock} onChange={(e) => setForm({ ...form, stock: Number(e.target.value) })} className={formStyle} />
        </div>
        <div>
          ...
          <tbody>
            ...
            products.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50 border-t">
                ...
                <td className="px-4 py-3">{CATS.find(c => c.v === p.category)?.l || '-'}</td>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setEditProduct(null)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md space-y-3" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input type="number" min={0} value={editProduct.price} onChange={(e) => setEditProduct({ ...editProduct, price: Number(e.target.value) })} className={formStyle} placeholder="Harga" />
              <input type="number" min={0} value={editProduct.stock} onChange={(e) => setEditProduct({ ...editProduct, stock: Number(e.target.value) })} className={formStyle} placeholder="Stok" />
            </div>
