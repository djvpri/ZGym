...
interface Ringkasan {
  ...
  memberRutin?: { nama: string; kunjungan: number }[]
  memberMenjelangExpired?: { nama: string; sisaHari: number }[]
  jamSibuk?: { jam: number; kunjungan: number }[]
  planTerlaris?: { nama: string; jumlah: number }[]
}
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Arahan & saran bisnis dari data gym Anda (30 hari terakhir). Maksimal 1x analisis per hari.
          </p>
        </div>
        <button
          onClick={bukaRiwayat}
      ...
      {r && !hasil?.error && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            <div className="text-xs text-gray-400 mb-1">Anggota Aktif</div>
            <div className="text-base font-semibold text-gray-900">{r.anggotaAktif ?? 0}</div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            <div className="text-xs text-gray-400 mb-1">Kunjungan 30 hari</div>
            <div className="text-base font-semibold text-gray-900">{r.totalKunjungan ?? 0}</div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            <div className="text-xs text-gray-400 mb-1">Pendapatan 30 hari</div>
            <div className="text-base font-semibold text-gray-900">{fmtRp(Number(r.totalPendapatan ?? 0))}</div>
          </div>
        </div>
      )}
      ...
      {!hasil && !loading && (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6">
          <div className="text-sm text-gray-500 leading-relaxed">
            Klik <b>Analisis Bisnis</b> untuk melihat saran tentang anggota yang paling aktif,
            anggota yang segera habis masa aktif (risiko churn), paket yang paling laku,
            serta jam operasional paling sibuk — berdasarkan data gym Anda. Analisis baru
            tersedia 1x per hari; hasilnya tersimpan di riwayat.
          </div>
        </div>
      )}
                      ...
                      <div>
                        ...
                        {item.ringkasan && (
                          <div className="text-xs text-gray-400 mt-0.5">
                            {item.ringkasan.totalKunjungan ?? 0} kunjungan · {fmtRp(Number(item.ringkasan.totalPendapatan ?? 0))}
                          </div>
                        )}
                      </div>
